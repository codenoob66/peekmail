...
$client = new Google\Client(['client_id' => '951139635404-1lg99r1qorsu66hu0lkr8ht767pbbihv.apps.googleusercontent.com']);
...
try {
    ...
    if ($payload) {
        ...
        if (!$user) {
            ...
            $stmt = $db->prepare("INSERT INTO users (google_id, email, username, password, name, picture) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$google_id, $email, $username, $hashed_password, $name, $picture]);
            ...
            if (empty($user['google_id'])) {
                $stmt = $db->prepare("UPDATE users SET google_id = ?, picture = ? WHERE id = ?");
